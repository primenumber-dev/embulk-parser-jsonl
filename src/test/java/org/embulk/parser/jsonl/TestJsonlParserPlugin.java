...
public class TestJsonlParserPlugin {
  ...
  public void skipRecords() throws Exception {
    ...
    transaction(config, fileInput("[]", "\"embulk\"", "10", "true", "false", "null", " "));
  ...
  public void throwDataException() throws Exception {
    SchemaConfig schema =
        schema(
            ...
            column("_c5", JSON));
    ConfigSource config =
        this.config.deepCopy().set("columns", schema).set("stop_on_invalid_record", true);

    try {
      transaction(config, fileInput("\"not_map_value\""));
      fail();
    } catch (Throwable t) {
  ...
  private ConfigSource getConfigFromYamlFile(File yamlFile) throws IOException {
